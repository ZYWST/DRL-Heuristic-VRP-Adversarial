...
class AgentController:
    def __init__(self, model_path: str, config: HyperConfig, problem_data: ProblemData):
        ...
        if not os.path.exists(vec_path):
             vec_path = os.path.join(os.path.dirname(model_path), "best_model_vecnormalize.pkl")
...
if __name__ == '__main__':
    ...
    # --- 1. 基础参数 (做了兼容性调整以适配批量脚本) ---
    # 允许位置参数 (旧习惯) 或 --problem 参数 (批量脚本习惯)
    parser.add_argument("problem_file_pos", type=str, nargs='?', help="算例路径 (位置参数)")
    parser.add_argument("--problem", type=str, help="算例路径 (flag参数)")
    
    parser.add_argument("--model", type=str, default="./logs/best_model.zip", help="模型路径")
    
    # 兼容 --gens (原版) 和 --generations (批量脚本)
    parser.add_argument("--gens", type=int, default=100, help="最大迭代代数")
    parser.add_argument("--generations", type=int, dest="gens_alias", help="最大迭代代数 (兼容别名)")
    
    parser.add_argument("--workers", type=int, default=None, help="CPU核数")
    parser.add_argument("--seed", type=int, default=None, help="随机种子 (用于复现)")
    
    # 兼容 --time_limit (原版) 和 --timeout (批量脚本)
    parser.add_argument("--time_limit", type=float, default=None, help="最大运行时间(秒)")
    parser.add_argument("--timeout", type=float, dest="time_limit_alias", help="最大运行时间(秒) (兼容别名)")
    
    parser.add_argument("--strict", action="store_true", help="开启严格中断模式(整车失效)")

    # --- 2. [新增] 敏感性分析参数 ---
    parser.add_argument("--sa_alpha", type=float, default=None, help="SA Cooling Rate")
    parser.add_argument("--aco_evaporation", type=float, default=None, help="ACO Evaporation Rate")
    parser.add_argument("--select_temp", type=float, default=None, help="Softmax Temperature")
    
    args = parser.parse_args()

    # --- 参数清洗与合并 ---
    # 优先使用 --problem, 其次使用位置参数
    problem_file = args.problem if args.problem else args.problem_file_pos
    if not problem_file:
        parser.error("必须指定算例文件路径 (通过位置参数 或 --problem)")

    # 合并 time_limit
    time_limit = args.time_limit_alias if args.time_limit_alias is not None else args.time_limit
    
    # 合并 generations
    generations = args.gens_alias if args.gens_alias is not None else args.gens

    if args.seed is not None:
        print(f"🎲 正在设置全局随机种子: {args.seed}")
        random.seed(args.seed)
        np.random.seed(args.seed)
        set_random_seed(args.seed)
    
    if not os.path.exists(problem_file):
        print(f"❌ 算例文件不存在: {problem_file}")
        sys.exit(1)

    hyper_config = HyperConfig()
    grasp_config = GraspConfig(PROBLEM_DATA_PATH=problem_file, NUM_WORKERS=args.workers)
    ...
    # --- 3. [新增] 动态覆盖配置参数 (用于敏感性分析) ---
    if args.sa_alpha is not None:
        grasp_config.SA_ALPHA = args.sa_alpha
        print(f"🔧 [Sensitivity] Override SA_ALPHA = {grasp_config.SA_ALPHA}")
        
    if args.aco_evaporation is not None:
        grasp_config.ACO_EVAPORATION = args.aco_evaporation
        print(f"🔧 [Sensitivity] Override ACO_EVAPORATION = {grasp_config.ACO_EVAPORATION}")
        
    if args.select_temp is not None:
        grasp_config.SELECT_TEMP = args.select_temp
        print(f"🔧 [Sensitivity] Override SELECT_TEMP = {grasp_config.SELECT_TEMP}")

    logger = setup_logger(grasp_config)

    logger.info(f"正在分析算例: {os.path.basename(problem_file)}")
    pd = ProblemData(problem_file, '', matrix_filepath="")
    ...
    try:
        optimizer.solve_with_agent(
            controller, 
            generations, # 使用合并后的代数
            time_limit=time_limit, # 使用合并后的时间限制
            seed=args.seed
        )
    except KeyboardInterrupt:
        logger.warning("用户强制中断，正在生成报告...")
        optimizer.save_solution_to_file(grasp_config.FINAL_SOLUTION_PATH)
        generate_detailed_report(optimizer, problem_file, "solution_report.txt", seed=args.seed)
    finally:
        optimizer.shutdown()

# python3 run_with_agent.py problem_instances/mid_scale/Case_Clustered_Seed71808.dat  --model logs/best_model.zip  --gens 200 --seed 42 --time_limit 1200